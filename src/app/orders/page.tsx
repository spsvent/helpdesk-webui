...
export default function OrdersPage() {
  ...
  const handleBulkConfirm = async (rows: QueueRow[], submission: BulkOrderSubmission) => {
    ...
    // Group selected rows by parent record. For each one build the new
    // lineItems array (preserve unselected items as-is, mutate the
    // selected ones with vendor/orderNum/actualCost/expectedDelivery).
    // Ticket rows and purchase-module rows are grouped separately — each
    // writes back to its own list — but rowOrder keeps the OVERALL index so
  ...
  const awaitingRows = [...flattenUnorderedItems(tickets), ...purchaseUnorderedRows(purchases)];
  const recentRows = [...flattenRecentlyOrdered(tickets, 30), ...purchaseRecentlyOrderedRows(purchases, 30)];
