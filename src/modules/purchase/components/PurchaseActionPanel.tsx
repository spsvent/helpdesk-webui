...
export default function PurchaseActionPanel({ pr, onMarkPurchased }: PurchaseActionPanelProps) {
  ...
  useEffect(() => {
    const d = loadDraft<{ orderItems?: PurchaseLineItem[]; notes?: string }>(`purchase:${pr.id}`);
    if (d) {
      if (d.orderItems) setOrderItems(d.orderItems);
      if (typeof d.notes === "string") setNotes(d.notes);
      setIsExpanded(true);
      clearDraft(`purchase:${pr.id}`);
    }
